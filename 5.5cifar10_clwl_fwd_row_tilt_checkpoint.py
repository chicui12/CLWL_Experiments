...
# resume/extend an earlier run from checkpoints, e.g. from 100 to 200 epochs
python cifar10_clwl_fwd_row_tilt_fixed.py \
  --data_root ./data --model resnet_small --epochs 200 --train_seeds 0 1 2 \
  --s_grid 0 0.25 0.5 0.75 1.0 --batch_size 128 --device cuda \
  --out_dir artifacts_cifar10_final --resume
"""
...
def _state_dict_to_cpu(state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Store checkpoints on CPU so they can be resumed on CPU or CUDA."""
    return {k: v.detach().cpu().clone() for k, v in state_dict.items()}


def _move_optimizer_state_to_device(opt: torch.optim.Optimizer, device: torch.device) -> None:
    """After loading an optimizer checkpoint, move its internal tensors to device."""
...
def _torch_load_checkpoint(path: Path) -> dict:
    """Compatibility wrapper for different PyTorch versions."""
    try:
...
def _save_training_checkpoint(
    ...
    *,
    ...
    """Atomically save enough state to resume training later."""
    ...
    payload = {
        ...
        "target_epochs_when_saved": int(epochs),
        "scheduler_t_max_when_saved": int(scheduler_t_max),
    ...
    if torch.cuda.is_available():
        try:
            ...
            payload["cuda_rng_state_all"] = None

    torch.save(payload, tmp_path)
    tmp_path.replace(ckpt_path)


def _restore_rng_states(ckpt: dict, device: torch.device) -> None:
    ...
    if device.type == "cuda" and "cuda_rng_state_all" in ckpt and ckpt["cuda_rng_state_all"] is not None:
        try:
...
def train_one(
    ...
    *, method: str, M_hat: Array, T_hat: Array, seed: int, model_name: str, epochs: int,
    ...
    effective_t_max = max(int(scheduler_t_max if scheduler_t_max is not None else epochs), 1)
    sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=effective_t_max)
    ...
    if resume and ckpt_path is not None and ckpt_path.exists():
        ckpt = _torch_load_checkpoint(ckpt_path)
        model.load_state_dict(ckpt["model_state"])
        opt.load_state_dict(ckpt["optimizer_state"])
        _move_optimizer_state_to_device(opt, dev)
        if "scheduler_state" in ckpt:
            sched.load_state_dict(ckpt["scheduler_state"])
            # If the user extends --epochs, keep the resumed scheduler but update
            # its total horizon. This gives a practical continuation from the
            # checkpoint; it is not identical to a fresh run that used the longer
            # horizon from epoch 1.
            if hasattr(sched, "T_max"):
                sched.T_max = effective_t_max
        start_epoch = int(ckpt["epoch"]) + 1
        best_val = float(ckpt.get("best_val", -1.0))
        best_epoch = int(ckpt.get("best_epoch", 0))
        best_state = ckpt.get("best_state", None)
        _restore_rng_states(ckpt, dev)
        print(f"[resume] {ckpt_path}: checkpoint epoch={start_epoch - 1}, target epochs={epochs}")

    if start_epoch > epochs:
        print(f"[skip] {ckpt_path}: already reached epoch {start_epoch - 1} >= target {epochs}")
    else:
        for ep in range(start_epoch, epochs + 1):
            ...
            if (
                ...
                and (ep % checkpoint_every == 0 or ep == epochs)
            ):
                _save_training_checkpoint(
                    ...
                    model_name=model_name,
                    epochs=epochs,
                    scheduler_t_max=effective_t_max,
                )
...
def run(args: argparse.Namespace) -> None:
    ...
    ckpt_dir = out / args.checkpoint_dir

    def s_tag(s: float) -> str:
        text = f"{float(s):.12g}".replace("-", "m").replace(".", "p")
        return text.replace("+", "")
    ...
    for split_seed in args.train_seeds:
        ...
        oracle = train_one(
            ...
            ckpt_path=ckpt_dir / f"oracle_seed{split_seed}.pt",
            resume=args.resume,
        ...
        for s in s_grid:
            ...
            clwl = train_one(
                ...
                batch_size=args.batch_size, lr=args.lr, weight_decay=args.weight_decay,
                device=device, augment=args.augment,
                ckpt_path=ckpt_dir / f"clwl_seed{split_seed}_s{s_tag(s)}.pt",
                resume=args.resume,
                checkpoint_every=args.checkpoint_every,
                scheduler_t_max=args.scheduler_t_max,
            )
            fwd = train_one(
                ...
                batch_size=args.batch_size, lr=args.lr, weight_decay=args.weight_decay,
                device=device, augment=args.augment,
                ckpt_path=ckpt_dir / f"fwd_seed{split_seed}_s{s_tag(s)}.pt",
                resume=args.resume,
                checkpoint_every=args.checkpoint_every,
                scheduler_t_max=args.scheduler_t_max,
            )
    ...
    summary["test_acc_mean_std"] = summary.apply(
        lambda r: f"{r['test_accuracy_mean']:.4f} ± {0.0 if pd.isna(r['test_accuracy_std']) else r['test_accuracy_std']:.4f}",
        axis=1,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--resume", action="store_true",
                   help="Resume each training job from its checkpoint if available.")
    p.add_argument("--checkpoint_dir", type=str, default="checkpoints",
                   help="Checkpoint subdirectory under --out_dir.")
    p.add_argument("--checkpoint_every", type=int, default=1,
                   help="Save checkpoint every N epochs. Use 0 to disable saving.")
    p.add_argument("--scheduler_t_max", type=int, default=None,
                   help="Optional T_max for CosineAnnealingLR. Defaults to --epochs.")
    return p.parse_args()
