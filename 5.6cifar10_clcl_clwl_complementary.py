#!/usr/bin/env python3
"""
CIFAR-10 complementary-label experiment for CLCL vs CLWL.

This script compares native complementary-label losses (CLCL-OR and a practical
CLCL-ORW weighted variant) with CLWL on CIFAR-10 under a complementary-label
transition matrix.

Weak-label protocol:
  For a clean class y, the observed weak label z is one CIFAR-10 class that the
  example does NOT belong to.

  with probability 1-rho: z is sampled uniformly among all classes except y;
  with probability rho:   z is sampled from a fixed non-uniform semantic-bias distribution over wrong labels.

Thus z is always complementary, even for rho>0.  rho controls how non-uniform and
class-dependent the complementary-label channel is.  rho=0 is the standard native
uniform complementary-label setting for CLCL.  Larger rho values create biased
complementary labels where CLWL can use the known transition matrix M_rho while
native CLCL still treats the observed complementary label directly.
...
Expected outputs in --out_dir:
  ...
  diagnostics.csv
  epoch_history.csv
  accuracy_vs_bias_probability.pdf/png
  loss_trend_over_epochs.pdf/png
  progress.txt

The files are updated after every completed (rho, seed, method) run, so progress
can be monitored during long GPU jobs.

Recommended paper-quality fast run on kumo/GPU:
  python cifar10_clcl_clwl_complementary_smallcnn_live.py \
      --data_root ./data \
      --out_dir artifacts_cifar10_clcl_clwl_comp_smallcnn \
      --model small_cnn \
      --cnn_width 32 \
      --epochs 60 \
      --rhos 0 0.15 0.3 0.45 0.6 \
      --seeds 0 1 2 \
      --methods clcl_or,clwl \
      --batch_size 256 \
      --optimizer adamw \
      --scheduler none \
      --lr 1e-3 \
...
Full appendix run with the weighted CLCL variant:
  python cifar10_clcl_clwl_complementary_smallcnn_live.py \
      --data_root ./data \
      --out_dir artifacts_cifar10_clcl_clwl_comp_smallcnn_orw \
      --model small_cnn --cnn_width 32 --epochs 60 \
      --rhos 0 0.15 0.3 0.45 0.6 --seeds 0 1 2 \
      --methods clcl_or,clcl_orw,clwl --batch_size 256 \
      --optimizer adamw --scheduler none --lr 1e-3 --weight_decay 1e-4 \
      --augment --amp --num_workers 4 --eval_every 10 --device cuda

Quick smoke test without CIFAR-10, not for reporting:
  python cifar10_clcl_clwl_complementary_live.py --fake_data --max_train 128 --max_test 64 --epochs 1 --rhos 0 0.3 --seeds 0 --methods clcl_or,clcl_orw,clwl --device cpu
"""
...
def build_complementary_model(rho: float, c: int = 10):
    """Build a biased complementary-label channel M_rho.

    M is c x c, column-stochastic, where M[z, y] = P(Z=z | Y=y).
    The diagonal is zero for all rho, so the observed weak label is always
    complementary.

    rho=0: uniform complementary labels.
    rho>0: mixture of uniform complementary labels and a fixed non-uniform,
           semantically biased complementary channel.
    """
    assert c == 10, "This CIFAR-10 protocol assumes c=10."
    if rho < 0 or rho > 1:
        raise ValueError("rho must lie in [0,1]")

    U = (np.ones((c, c), dtype=np.float64) - np.eye(c, dtype=np.float64)) / (c - 1)
    B, pi = build_semantic_bias_matrix(c=c)
    M = (1.0 - rho) * U + rho * B
...
def train_one(args, Xtr, ytr, Xte, yte, ztr, T, method: str, rho: float, seed: int):
    ...
    final_train_acc = evaluate(model, train_eval_loader, device, channels_last=args.channels_last)
    final_test_acc = evaluate(model, test_loader, device, channels_last=args.channels_last)
...
def plot_accuracy_vs_bias(summary: pd.DataFrame, out_base: Path):
    ...
    for method in method_order:
        ...
        ax.errorbar(sub["rho"], sub["test_acc_mean"], yerr=yerr,
                    marker="o", capsize=3, label=label_map.get(method, method.upper()))
    ax.set_xlabel(r"Biased complementary probability $\rho$")
    ax.set_ylabel("Test accuracy")
    ax.set_title("CIFAR-10: CLCL vs. CLWL under complementary labels")
...
def save_live_outputs(rows, epoch_rows, out_dir: Path, args, completed_runs: int, final: bool = False) -> pd.DataFrame:
    ...
    if not raw_df.empty:
        ...
        plot_accuracy_vs_bias(summary, out_dir / "accuracy_vs_bias_probability")
    else:
    ...
    print(
        ...
        f"epoch_history.csv, accuracy_vs_bias_probability.pdf/png, loss_trend_over_epochs.pdf/png",
        flush=True,
    )
    return summary
...
def parse_args():
    ...
    p.add_argument("--out_dir", type=str, default="artifacts_cifar10_clcl_clwl_comp")
    ...
    p.add_argument("--rhos", type=float, nargs="+", default=[0.0, 0.15, 0.3, 0.45, 0.6])
    p.add_argument("--seeds", type=int, nargs="+", default=[0, 1, 2])
    p.add_argument("--methods", type=str, default="clcl_or,clwl", help="Comma-separated subset of clcl_or,clcl_orw,clwl")
...
def main():
    ...
    print(f"Saved plot: {out_dir / 'accuracy_vs_bias_probability.pdf'}")
