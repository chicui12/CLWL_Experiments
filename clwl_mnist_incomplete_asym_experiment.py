...
def train_one(Xtr, ytr, Xte, yte, rho, method, seed, epochs, lr, hidden, weight_decay):
    ...
    for _ in range(epochs):
        ...
        if method == "clpl":
            ...
            loss = (F.softplus(-mean_s) + ((1.0 - b) * F.softplus(s)).sum(dim=1)).mean()
        elif method == "clwl":
            tz = T_rows[ztr_t]
            loss = (tz * F.softplus(-s) + (1.0 - tz) * F.softplus(s)).sum(dim=1).mean()
        else:
    ...
    with torch.no_grad():
        pred_te = model(Xte_t).argmax(dim=1).cpu().numpy()
        pred_tr = model(Xtr_t).argmax(dim=1).cpu().numpy()

    return accuracy_score(yte, pred_te), accuracy_score(ytr, pred_tr)
